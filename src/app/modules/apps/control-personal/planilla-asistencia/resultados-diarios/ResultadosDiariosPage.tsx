...
import {getDetalleResultadoDiario, getProcesosPlanilla, getResultadosDiarios} from '../core/_requests'
...
const renderHorario = (horario?: string | null, nombreHorarioTipo?: string | null, idHorarioTipo?: number | null, justificativo?: string | null) => {
  const horarioBase = horario || nombreHorarioTipo || (idHorarioTipo ? `Horario ${idHorarioTipo}` : '-')
...
const ResultadosDiariosPage = () => {
  ...
  const cargar = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || err?.message || 'No se pudo cargar resultados diarios.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const verDetalle = async (row: ResultadoDiario) => {
    if (!canLoad || !row.id_persona || !row.fecha || (row.estado_dia || '').toUpperCase() === 'NO_LABORABLE') {
      return
    }
    ...
    try {
      ...
      setError(err?.response?.data?.message || err?.message || 'No se pudo cargar el detalle diario.')
    }
  }
        ...
        <div className='row g-5 mb-7'>
          <div className='col-12 col-md-3'>
            <div className='card card-flush h-100'>
              <div className='card-body'>
                <div className='text-gray-500 fs-7 text-uppercase fw-bold mb-2'>Registros diarios</div>
          ...
          <div className='col-12 col-md-3'>
            <div className='card card-flush h-100'>
              <div className='card-body'>
                <div className='text-gray-500 fs-7 text-uppercase fw-bold mb-2'>Atrasos detectados</div>
          ...
          <div className='col-12 col-md-3'>
            <div className='card card-flush h-100'>
              <div className='card-body'>
                <div className='text-gray-500 fs-7 text-uppercase fw-bold mb-2'>Faltas y abandonos</div>
                <div className='fs-1 fw-bolder text-danger'>
                  {
                    rows.filter((row) =>
                      ['FALTA', 'ABANDONO', 'SIN_MARCACION'].includes((row.estado_dia || '').toUpperCase())
                    ).length
                  }
                </div>
              </div>
          ...
          <div className='col-12 col-md-3'>
            <div className='card card-flush h-100'>
              <div className='card-body'>
                <div className='text-gray-500 fs-7 text-uppercase fw-bold mb-2'>Minutos de atraso</div>
                <div className='fs-1 fw-bolder text-primary'>
                  {rows.reduce((sum, row) => sum + (row.minutos_atraso_oficial ?? row.minutos_atraso_calculado ?? 0), 0)}
                </div>
              </div>
            </div>
          </div>
                ...
                <tbody className='fw-semibold text-gray-700'>
                  {rows.map((row, index) => (
                    <tr key={`${row.id_persona}-${row.fecha}-${index}`}>
                      ...
                      <td>
                        <div className='d-flex flex-column gap-1'>
                          <div><StatusBadge value={row.estado_dia} /></div>
                          {row.justificativo_principal ? (
                            <div>
                              <span className='badge badge-light-info'>{row.justificativo_principal}</span>
                            </div>
                          ) : null}
                        </div>
                      </td>
                      ...
                      <td>
                        {row.cantidad_marcaciones_validas ?? 0} / {row.cantidad_marcaciones_esperadas ?? 0}
                      </td>
              ...
              >
                <div className='modal-header'>
                  <div className='d-flex flex-column'>
                    ...
                    <span className='text-muted mt-1'>
                      {detalle.persona?.nombre_completo || `Persona ${detalle.id_persona}`} - {detalle.fecha}
                    </span>
                  </div>
                  <button type='button' className='btn btn-sm btn-icon btn-active-light-primary' onClick={closeDetalle}>
                ...
                <div className='modal-body'>
                  <div className='row g-5 mb-7'>
                    <div className='col-12 col-md-3'>
                      <div className='card card-flush border h-100'>
                        <div className='card-body'>
                          <div className='text-gray-600 fs-7 text-uppercase mb-2'>Estado del día</div>
                          <div className='d-flex flex-column gap-2'>
                            <div><StatusBadge value={detalle.estado_dia} /></div>
                            {detalle.justificativo_principal ? (
                              <div>
                                <span className='badge badge-light-info'>{detalle.justificativo_principal}</span>
                              </div>
                            ) : null}
                          </div>
                        </div>
                    ...
                    <div className='col-12 col-md-3'>
                      <div className='card card-flush border h-100'>
                        <div className='card-body'>
                          <div className='text-gray-600 fs-7 text-uppercase mb-2'>Horario aplicado</div>
                    ...
                    <div className='col-12 col-md-3'>
                      <div className='card card-flush border h-100'>
                        <div className='card-body'>
                          <div className='text-gray-600 fs-7 text-uppercase mb-2'>Atraso sancionable</div>
                          <div className='fw-bold fs-5'>
                            {detalle.minutos_atraso_oficial ?? detalle.minutos_atraso_calculado ?? 0} min
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className='col-12 col-md-3'>
                      <div className='card card-flush border h-100'>
                        <div className='card-body'>
                          <div className='text-gray-600 fs-7 text-uppercase mb-2'>Marcaciones válidas</div>
                          <div className='fw-bold fs-5'>
                            {detalle.cantidad_marcaciones_validas ?? 0} / {detalle.cantidad_marcaciones_esperadas ?? 0}
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className='alert alert-light-info border border-info border-dashed mb-7'>
                    <span className='fw-semibold'>Observación del día:</span> {detalle.observacion || 'Sin observación.'}
                  </div>
                              ...
                              <tr key={`${punto.codigo_punto || punto.nombre_punto}-${index}`}>
                                ...
                                <td>
                                  <div className='d-flex flex-column'>
                                    ...
                                    <span className='text-muted fs-7'>{punto.codigo_punto || '-'}</span>
                                  </div>
                                </td>
                                <td>{punto.hora_esperada || '-'}</td>
                                <td>{punto.hora_marcada || punto.justificativo_punto || '-'}</td>
                                <td><StatusBadge value={punto.tipo_resultado} /></td>
                  ...
                  <div className='row g-7 mb-8'>
                    <div className='col-12 col-lg-6'>
                      <div className='card card-flush border h-100'>
                        ...
                        <div className='card-body pt-4'>
                          {detalle.detalle_json?.marcaciones && detalle.detalle_json.marcaciones.length > 0 ? (
                            <div className='table-responsive'>
                            ...
                            <div className='text-muted'>Sin marcaciones crudas para este día.</div>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className='col-12 col-lg-6'>
                      <div className='card card-flush border h-100'>
                        ...
                        <div className='card-body pt-4'>
                          {detalle.detalle_json?.marcaciones_sobrantes && detalle.detalle_json.marcaciones_sobrantes.length > 0 ? (
                            <div className='table-responsive'>
                              <table className='table align-middle table-row-dashed fs-7 gy-3 mb-0'>
                                <thead>
                                  <tr className='text-start text-muted fw-bold fs-8 text-uppercase gs-0'>
                                ...
                                <tbody>
                                  {detalle.detalle_json.marcaciones_sobrantes.map((marcacion, index) => (
                  ...
                  <div className='row g-7'>
                    <div className='col-12 col-lg-6'>
                      <div className='card card-flush border h-100'>
                        ...
                        <div className='card-body pt-4'>
                          <div className='mb-4'>
                            <div className='text-gray-600 fs-7 text-uppercase mb-1'>Justificativo</div>
                            ...
                            <div className='fw-semibold'>
                              {formatList(
                                detalle.detalle_json?.guardias?.map(
                                  (guardia) =>
                                    `${guardia.nombre_turno || 'Turno'} ${guardia.hora_inicio || '-'} - ${guardia.hora_fin || '-'}`
                                )
                              )}
                            </div>
                          </div>
                          <div>
                            <div className='text-gray-600 fs-7 text-uppercase mb-1'>No descontable</div>
                            <div className='fw-semibold'>
                    ...
                    <div className='col-12 col-lg-6'>
                      <div className='card card-flush border h-100'>
                        ...
                        <div className='card-body pt-4'>
                          {detalle.detalle_json?.reemplazos_titular && detalle.detalle_json.reemplazos_titular.length > 0 ? (
                            <div className='table-responsive'>
                              <table className='table align-middle table-row-dashed fs-7 gy-3 mb-0'>
                                <thead>
                                  <tr className='text-start text-muted fw-bold fs-8 text-uppercase gs-0'>
                                ...
                                <tbody>
                                  {detalle.detalle_json.reemplazos_titular.map((reemplazo, index) => (
                                    <tr key={`${reemplazo.id_guardia_reemplazo || 'r'}-${index}`}>
                                      <td>{reemplazo.id_guardia_reemplazo || '-'}</td>
                                      <td>{reemplazo.fecha || '-'}</td>
                                      <td>{`${reemplazo.hora_inicio || '-'} - ${reemplazo.hora_fin || '-'}`}</td>
                                      <td>
                                        <div className='d-flex flex-column'>
                            ...
                            <div className='text-muted'>Sin reemplazos de titular para este día.</div>
                          )}
                        </div>
                      </div>
                    </div>

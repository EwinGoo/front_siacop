...
const ComisionModal: React.FC<ModuleModalProps> = ({
  data,
  accionesDisponibles,
  ...
  const title = (
    <div className='text-center'>
      <h4 className='text-info fw-bold mb-1'>
        <i className='bi bi-briefcase me-2'></i>PERMISO POR HORA
      </h4>
  ...
  return (
        ...
        <>
          ...
          {accionesDisponibles.includes('reception') && (
            <button className='btn btn-success btn-sm' onClick={() => handleAction('reception')}>
              <i className='bi bi-check-circle me-1'></i>Recepcionar Comisión
            </button>
          )}
          ...
          {accionesDisponibles.includes('approve') && (
            <button className='btn btn-success btn-sm' onClick={() => handleAction('approve')}>
              <i className='bi bi-check-circle me-1'></i>Aprobar Comisión
            </button>
          )}
        </>
      }
    >
      ...
      <div className='bg-info bg-opacity-10 p-3 rounded'>
        <h6 className='text-info mb-2 fw-semibold'>
          <i className='bi bi-briefcase me-2'></i>Detalles de la Comisión
        </h6>
        <div className='row g-1 small'>
          ...
          {data.hora && (
            <>
              ...
              <div className='col-8'>
                : <span className='badge bg-secondary'>{data.hora}</span>
              </div>
            </>
          )}
          ...
          {data.recorrido_de && (
            <>
              <div className='col-4 fw-bold'>De</div>
              <div className='col-8'>: {data.recorrido_de}</div>
            </>
          )}

          {data.recorrido_a && (
            <>
              <div className='col-4 fw-bold'>A</div>
              <div className='col-8'>: {data.recorrido_a}</div>
            </>
          )}

          {data.descripcion && (
            <>
              <div className='col-4 fw-bold'>Descripción</div>
              <div className='col-8'>: {data.descripcion}</div>
            </>
          )}

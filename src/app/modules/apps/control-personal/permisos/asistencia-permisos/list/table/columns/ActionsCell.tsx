...
import {deleteAsistenciaPermiso, toggleAsistenciaPermisoStatus} from '../../core/_requests'
...
const ActionsCell: FC<Props> = ({id, isActive}) => {
  ...
  const toggleStatus = useMutation(() => toggleAsistenciaPermisoStatus(id), {
    onSuccess: () => {
      queryClient.invalidateQueries([`${QUERIES.ASISTENCIAS_PERMISO_LIST}-${query}`])
      const action = isActive ? 'rechazado' : 'aprobado'
      toast.success(`Permiso ${action} correctamente`, {
        position: 'top-right',
        autoClose: 5000,
    ...
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Error al cambiar el estado del permiso', {
        position: 'top-right',
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: false,
  ...
  const handleToggleStatus = async () => {
    try {
      const action = isActive ? 'Aprobar' : ''
      const result = await Swal.fire({
        title: `${action} permiso?`,
        text: `Esta acción ${action.toLowerCase()}á el permiso`,
        icon: 'question',
        ...
        {isActive && (
          <div className='menu-item px-3'>
            <a className='menu-link px-3' onClick={handleToggleStatus}>
              <KTIcon iconName={isActive ? 'toggle-off' : 'toggle-on'} className='fs-5 me-2' />
              {/* {isActive ? 'Rechazar' : 'Aprobar'} */}
              Aprobar
            </a>
          </div>
        )}

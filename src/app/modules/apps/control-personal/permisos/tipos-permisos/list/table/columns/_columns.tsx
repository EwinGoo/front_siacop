...
const Columns: ReadonlyArray<Column<TipoPermiso>> = [
  ...
  {
    Header: (props) => <CustomHeader tableProps={props} title='Nombre' className='min-w-200px' />,
    id: 'nombre',
    Cell: ({...props}) => <InfoCell tipoPermiso={props.data[props.row.index]} />,
  },
  {
    Header: (props) => (
      <CustomHeader tableProps={props} title='Requiere Hoja Ruta' className='min-w-150px' />
    ),
    accessor: 'requiere_hoja_ruta',
    Cell: ({value}) => (
      <span className={`badge badge-light-${value == '1' ? 'success' : 'warning'}`}>
        {value == '1' ? 'Sí' : 'No'}
      </span>
    ),
  },
  {
    ...
    Cell: ({value}) => value ?? <span className='text-muted'>No definido</span>,
  },
  {
